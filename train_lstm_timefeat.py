...
def get_feature_columns(df: pd.DataFrame) -> list[str]:
    base_cols = [col for col in df.columns if col not in DROP_COLS]
    # 确保weekday和month被包含在特征中（如果存在）
    extra_cols = ["weekday", "month"]
    return base_cols + [col for col in extra_cols if col in df.columns]
...
def build_windows(
    ...
    for start in range(start_idx, end_idx + 1):
        x = feat_array[start : start + input_len]
        y = target_array[start + input_len : start + input_len + output_len]
        xs.append(x)
        ys.append(y)
...
def save_prediction_plot(results: dict[int, dict], output_path: str):
    ...
    for ax, horizon in zip(axes, [90, 365]):
        ...
        ax.plot(pred_curve, label="LSTM + Time Features Prediction", color="tomato", alpha=0.85)
        ax.set_title(f"LSTM + Time Features Rolling Forecast (horizon = {horizon} days)")
...
def main():
    ...
    print("Added time features: ['weekday', 'month']")

    results = {}

    for horizon in [90, 365]:
        print("\n" + "=" * 70)
        print(f"LSTM + Time Features direct multi-step forecasting: output_len={horizon}")
        print("=" * 70)
    ...
    if results:
        ...
        plot_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "results","lstm_timefeat_prediction.png"
        )
