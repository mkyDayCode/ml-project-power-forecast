...
class SimpleCNN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.permute(0, 2, 1)
        out = self.conv(x)  # (B, output_channels, 1)
        out = out.squeeze(-1)  # (B, output_channels)
        return self.proj(out)  # (B, output_channels)


class CNNPeriodicTransformer(nn.Module):
    """
    单尺度CNN + 周期感知嵌入 + Transformer 改进模型
    """
    def __init__(
        self,
        feature_size: int,
        ...
        super().__init__()
        # 1. CNN分支：提取局部时序模式
        self.cnn = SimpleCNN(feature_size, cnn_out_channels, kernel_size)
    ...
    def forward(self, x: torch.Tensor, weekday: torch.Tensor, month: torch.Tensor) -> torch.Tensor:
        B, seq_len, _ = x.shape

        # 1. CNN提取局部特征
        cnn_feat = self.cnn(x)  # (B, cnn_out_channels)
        ...
        return self.head(last_state)  # (B, output_len)
...
def inverse_metrics(
    ...
    with torch.no_grad():
        for (x, weekday, month), y in loader:
            pred = model(x.to(DEVICE), weekday.to(DEVICE), month.to(DEVICE)).cpu().numpy()
            preds.append(pred)
...
def save_prediction_plot(results: dict[int, dict], output_path: str):
    ...
    for ax, horizon in zip(axes, [90, 365]):
        ...
        ax.plot(pred_curve, label="CNN-Periodic-Transformer", color="tomato", alpha=0.85)
        ax.set_title(f"CNN-Periodic-Transformer Rolling Forecast (horizon = {horizon} days)")
...
def main():
    ...
    for horizon in [90, 365]:
        print("\n" + "=" * 70)
        print(f"CNN-Periodic-Transformer direct multi-step forecasting: output_len={horizon}")
        print("=" * 70)
        print("Metric protocol: compute MSE/MAE for each test window first, then average across windows.")
    ...
    if results:
        ...
        for horizon, result in results.items():
            summary_rows.append(
                {
                    "Model": "CNN-Periodic-Transformer",
        ...
        plot_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "results", "one_cnn_periodic_transformer.png"
        )
