...
class SequenceDataset(Dataset):
    def __init__(self, features: np.ndarray, targets: np.ndarray,
                 weekdays: np.ndarray, months: np.ndarray):
        self.features = torch.tensor(features, dtype=torch.float32)
        self.targets = torch.tensor(targets, dtype=torch.float32)
        self.weekdays = torch.tensor(weekdays, dtype=torch.long)
        self.months = torch.tensor(months, dtype=torch.long)

    def __len__(self) -> int:
        return len(self.features)

    def __getitem__(self, idx: int):
        return (self.features[idx], self.weekdays[idx], self.months[idx]), self.targets[idx]
...
class AttentionCNN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.permute(0, 2, 1)
        # CNN提取局部特征
        conv_out = self.conv(x)  # (B, output_channels, seq_len)
        # 转回序列形式
        conv_out = conv_out.permute(0, 2, 1)  # (B, seq_len, output_channels)
        # 注意力权重
        attn_weights = self.attention(conv_out)  # (B, seq_len, 1)
        attn_weights = torch.softmax(attn_weights, dim=1)  # 在序列维度上归一化
        # 加权聚合：按注意力权重对序列进行加权求和
        weighted = (conv_out * attn_weights).sum(dim=1)  # (B, output_channels)
        return self.proj(weighted), attn_weights.squeeze(-1)  # 返回特征和注意力权重（用于可视化）


class AttnCNNPeriodicTransformer(nn.Module):
    """
    注意力CNN + 周期感知嵌入 + Transformer
    """
    def __init__(
        self,
        feature_size: int,
        ...
        # 2. 周期嵌入分支
        self.week_embedding = nn.Embedding(7, WEEK_EMB_DIM)
        ...
        # 4. 融合层
        fusion_input_dim = d_model + WEEK_EMB_DIM + MONTH_EMB_DIM
    ...
    def forward(self, x: torch.Tensor, weekday: torch.Tensor, month: torch.Tensor):
        B, seq_len, _ = x.shape

        # 1. 注意力CNN提取局部特征 + 注意力权重
        cnn_feat, attn_weights = self.cnn(x)  # cnn_feat: (B, cnn_out_channels), attn_weights: (B, seq_len)
        cnn_feat = self.cnn_proj(cnn_feat)  # (B, d_model)
        # 扩展为序列长度
        cnn_feat = cnn_feat.unsqueeze(1).expand(-1, seq_len, -1)  # (B, seq_len, d_model)

        # 2. 位置编码
        cnn_feat = self.pos_encoder(cnn_feat)  # (B, seq_len, d_model)

        # 3. 周期嵌入
        week_emb = self.week_embedding(weekday)  # (B, seq_len, 16)
        month_emb = self.month_embedding(month)  # (B, seq_len, 16)

        # 4. 融合
        fused = torch.cat([cnn_feat, week_emb, month_emb], dim=-1)  # (B, seq_len, 64+16+16=96)
        fused = self.fusion(fused)  # (B, seq_len, 64)
        fused = self.norm(fused)
        fused = self.dropout(fused)

        # 5. Transformer编码
        encoded = self.transformer_encoder(fused)  # (B, seq_len, 64)

        # 6. 取最后一步 + 输出
        last_state = encoded[:, -1, :]  # (B, 64)
        return self.head(last_state), attn_weights  # 返回预测值和注意力权重（用于可视化）
...
def extract_temporal_info(df: pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
    """提取 weekday(0-6) 和 month(0-11)"""
    weekdays = df["datetime"].dt.dayofweek.values.astype(np.int64)
    months = df["datetime"].dt.month.values.astype(np.int64) - 1
...
def build_windows(
    ...
    for start in range(start_idx, end_idx + 1):
        ...
        ws.append(weekday_array[start : start + input_len])
        ms.append(month_array[start : start + input_len])

    if not xs:
        return (
            np.empty((0, input_len, feat_array.shape[1]), dtype=np.float32),
            np.empty((0, output_len), dtype=np.float32),
            np.empty((0, input_len), dtype=np.int64),
            np.empty((0, input_len), dtype=np.int64),
        )
    return (
...
def prepare_train_dataset(
    ...
    weekdays, months = extract_temporal_info(train_df)
    x, y, w, m = build_windows(feat, target, weekdays, months, INPUT_LEN, output_len)
    return SequenceDataset(x, y, w, m)
...
def prepare_eval_dataset(
    ...
    weekdays, months = extract_temporal_info(full_df)
    x, y, w, m = build_windows(
        feat,
        target,
        weekdays,
        months,
...
def train_one_epoch(model, loader, optimizer, criterion):
    model.train()
    total_loss = 0.0
    for (x, weekday, month), y in loader:
        x = x.to(DEVICE)
        weekday = weekday.to(DEVICE)
        month = month.to(DEVICE)
        y = y.to(DEVICE)
        optimizer.zero_grad()
        pred, _ = model(x, weekday, month)  # 忽略注意力权重
...
def run_training(
    ...
    model = AttnCNNPeriodicTransformer(
...
def inverse_metrics(
    ...
    with torch.no_grad():
        for (x, weekday, month), y in loader:
            pred, _ = model(x.to(DEVICE), weekday.to(DEVICE), month.to(DEVICE))
...
def save_prediction_plot(results: dict[int, dict], output_path: str):
    ...
    for ax, horizon in zip(axes, [90, 365]):
        ...
        ax.plot(pred_curve, label="AttnCNN-Periodic-Transformer", color="tomato", alpha=0.85)
        ax.set_title(f"AttnCNN-Periodic-Transformer Rolling Forecast (horizon = {horizon} days)")
...
def main():
    ...
    print("Using Attention CNN + Periodic Embedding")

    results = {}

    for horizon in [90, 365]:
        print("\n" + "=" * 70)
        print(f"AttnCNN-Periodic-Transformer direct multi-step forecasting: output_len={horizon}")
        print("=" * 70)
    ...
    if results:
        ...
        for horizon, result in results.items():
            summary_rows.append(
                {
                    "Model": "AttnCNN-Periodic-Transformer",
        ...
        plot_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "results", "attn_cnn_periodic_transformer.png"
        )
