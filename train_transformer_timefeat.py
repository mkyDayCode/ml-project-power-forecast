...
def save_prediction_plot(results: dict[int, dict], output_path: str):
    ...
    for ax, horizon in zip(axes, [90, 365]):
        ...
        ax.plot(pred_curve, label="Transformer + TimeFeat Prediction", color="tomato", alpha=0.85)
        ax.set_title(f"Transformer + TimeFeat Rolling Forecast (horizon = {horizon} days)")
...
def main():
    ...
    for horizon in [90, 365]:
        print("\n" + "=" * 70)
        print(f"Transformer + TimeFeat direct multi-step forecasting: output_len={horizon}")
        print("=" * 70)
    ...
    if results:
        ...
        plot_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)),"results","transformer_timefeat_prediction.png"
        )
